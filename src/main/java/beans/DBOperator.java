...
public class DBOperator {
    ...
    public static List<ResultsEntityManager> getAllResultsDetails() {
        Query queryObj = entityMgrObj.createQuery("SELECT s FROM ResultsEntityManager s");
    ...
    public static String updateResultDetails(long resultId, double x, double y, double r, String hitting, long time) {
        ...
        if(isResultIdPresent(resultId)) {
            Query queryObj = entityMgrObj.createQuery("UPDATE ResultsEntityManager s SET s.date=:date, s.x=:x, s.y=:y, s.r=:r, s.hit=:hit, s.time=:time WHERE s.id= :id");
    ...
    private static boolean isResultIdPresent(long resultId) {
        ...
        Query queryObj = entityMgrObj.createQuery("SELECT s FROM ResultsEntityManager s WHERE s.id = :id");
        queryObj.setParameter("id", resultId);
